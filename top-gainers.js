...
const config = {
  ...
  nocodb: {
    ...
    tableId: 'mio6tvuhceq5j22',
    viewId: 'vw2rh13odg84oreb'
  }
};
...
const TICKER_TO_COINGECKO_ID = {
  'LVLUSD': 'level-usd',
  'USDE': 'ethena',
  'USDM': 'mountain-protocol', // Fixed: changed from mountain-protocol-usd to mountain-protocol
  'USDS': 'usds',
  'FDUSD': 'first-digital-usd',
  'USDL': 'lift-dollar',
  'FUSD': 'usdf', // Fixed: changed from falcon-usd to usdf
  'USDY': 'ondo-us-dollar-yield',
  'GHO': 'gho',
  'MUSD': 'meta-usd' // Fixed: changed from mstable-usd to meta-usd
};
...
async function fetchCoinGeckoData(coinId) {
  try {
    ...
    // Add fallback logic for specific problematic coins
    if (coinId === 'mountain-protocol' || coinId === 'usdf' || coinId === 'meta-usd') {
      console.log(`Using alternative approach for ${coinId}...`);
      
      // First try to get the coin data using the /coins/{id} endpoint
      try {
        const response = await axios.get(
        ...
        // If that fails, try the /coins/markets endpoint which might have the token
        ...
        if (marketsResponse.data && marketsResponse.data.length > 0) {
          const coinData = marketsResponse.data[0];
          
          // Check for the 7-day market cap change in the markets response
          let marketCapChange7d = coinData.market_cap_change_percentage_7d;
          
          // If that's null, try price_change_percentage_7d_in_currency
          if (marketCapChange7d === null || marketCapChange7d === undefined) {
            marketCapChange7d = coinData.price_change_percentage_7d_in_currency;
          }
          
          // If that's null, try price_change_percentage_7d
          if (marketCapChange7d === null || marketCapChange7d === undefined) {
            marketCapChange7d = coinData.price_change_percentage_7d;
          }
          ...
          // If we still can't find it, try one more approach - use search
          ...
          if (searchResponse.data && 
            ...
            if (newId !== coinId) {
              return await fetchCoinGeckoData(newId);
            }
          }
          
          console.log(`All fallbacks failed for ${coinId}`);
          return {
            volume_24h: null,
    ...
    // Standard approach for most coins
    const response = await axios.get(
      `${config.coingecko.baseUrl}/coins/${coinId}`,
      {
        params: {
