...
// Tracks @aixbt_agent mentions and updates NocoDB with just total mentions and timestamp
...
// Configuration
const TARGET_ACCOUNT = 'fartcoinbase';
...
const USER_DATA_DIR = path.join(__dirname, 'chrome_profile_basedfartcoin'); // Different directory name
...
const logFilePath = path.join(OUTPUT_DIRECTORY, `basedfartcoin_tracker_log_${sessionId}.json`);
...
async function updateNocoDB(totalMentions) {
  try {
    ...
    const data = {
      ...
      token_name: 'Based Fartcoin',
      token_id: '0x2f6c17fa9f9bc3600346ab4e48c0701e1d5962ae',
